...
const SuggestionsInbox = () => {
  const pageSize = 3;
  const numberOfPages = Math.ceil(useSelector(selectNumberOfSuggestions) / pageSize);
  const [currentPage, setCurrentPage] = useState(1);
  const suggestions = useSelector(selectSuggestionsByRange((currentPage - 1) * pageSize, currentPage * pageSize));
  ...
  return (
    <SuggestionInboxStyled data-cy="suggestions-inbox">
      <Icon icon="arrow_back_ios" onClick={() => setCurrentPage(getNextPage(-1))}/>
      {suggestions.map(({ userName, description, subject, id }, index) => (
        <SuggestionCard
          ...
          userName={userName}
        />))}
      <Icon icon="arrow_forward_ios" onClick={() => setCurrentPage(getNextPage(1))}/>
